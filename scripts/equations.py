...
def main():
    ...
    for i in range(n_line):
        ...
        z_base = base_kV[sub_from] * base_kV[sub_to] / baseMVA  # pu, the k^2 / M cancels out
        ...
        if sub_from_to in net_line_from_to:
            ...
            z_k_mod = net.trafo["vk_percent"] / 100 * baseMVA / net.trafo["sn_mva"]
            r_k = net.trafo["vkr_percent"] / 100 * baseMVA / net.trafo["sn_mva"]
            x_k = np.sqrt(z_k_mod**2 - r_k**2)
            z_k = r_k + 1j * x_k
            ...
            # TODO meni kao da je ovo baseMVA ovde viska
            z_ref = net.trafo["vn_lv_kv"] ** 2 * baseMVA / net.trafo["sn_mva"]

            z = (z_k * z_ref / z_base)[trafo_idx]

            y = (y_m * z_base / z_ref)[trafo_idx]

            n_mod = net.trafo["vn_hv_kv"] / net.trafo["vn_lv_kv"] * base_kV[sub_to] / base_kV[sub_from]
    ...
    for line_f_idx in line_f_ids_at_sub:
        ...
        assert round(Pf_line, num_dec) == round(obs.p_or[line_f_idx] / baseMVA, num_dec)
        assert round(Qf_line, num_dec) == round(obs.q_or[line_f_idx] / baseMVA, num_dec)
    ...
    for line_t_idx in line_t_ids_at_sub:
        ...
        # TODO
        #  nesto nije u redu ovde
        #  line_t_idx = 17 znaci trafo

        # TODO mozda grid2op ima drugacija podesavanja za trafo. Mozda je Pi model
        assert round(Pt_line, num_dec) == round(obs.p_ex[line_t_idx] / baseMVA, num_dec)
        assert round(Qt_line, num_dec) == round(obs.q_ex[line_t_idx] / baseMVA, num_dec)
